 ...
 *
 */
contract ButtonTokenClone is IButtonToken, Initializable, OwnableUpgradeable {
    ...
    uint256 private constant TOTAL_BITS =
        MAX_UINT256 - (MAX_UINT256 % MAX_UNDERLYING);
    ...
    uint256 private constant PRICE_BITS =
        BITS_PER_UNDERLYING * (10 ** PRICE_DECIMALS);
    ...
    function initialize(
        ...
        require(
            ...
            "ButtonToken: invalid underlying reference"
        );
    ...
    function _queryPrice() private view returns (uint256, bool) {
        ...
        return (
            valid && newPrice > 0 ? newPrice : lastPrice,
            valid && newPrice > 0
        );
    }

    /// @dev Convert button token amount to bits.
    function _amountToBits(
        uint256 amount,
        uint256 price
